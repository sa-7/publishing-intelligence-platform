...
app.post('/api/chat/send', async (req, res) => {
    ...
    try {
        ...
        // Always try real data analysis first since OpenAI is failing
        console.log('📊 Generating response from real data...');
        aiResponse = await generateRealDataResponse(message, assistantType);
        
        if (!aiResponse || aiResponse.trim() === '') {
            console.log('⚠️ Real data response was empty, trying OpenAI...');
            ...
            if (openai) {
                try {
                    ...
                    console.error('❌ OpenAI API error:', openaiError.message);
                    aiResponse = `I'm analyzing your question using real subscription data...

The OpenAI service is temporarily unavailable, but I can still provide insights based on your actual Excel data. Let me try a direct analysis of your question.`;
                }
            }
        }
        
        // If still no response, provide a basic fallback
        if (!aiResponse || aiResponse.trim() === '') {
            aiResponse = `I'm having trouble processing your request, but I can see you're asking about "${message}". 

The system has loaded data from your Excel files:
...
- "university comparison"`;
        ...
        res.status(500).json({ 
            error: 'Failed to process request',
            details: error.message,
            response: `Sorry, I encountered an error processing your request. The system has loaded data from your Excel files but there was a technical issue. Please try a simpler question.`
        });
    }
});
